...
`Integrations::Cj::Adapter#product_list(page:, page_size:, category: nil, keyword: nil)` returns one bounded page (page size capped at `Adapter::MAX_PAGE_SIZE`, currently 200) of product summaries for catalog discovery (Product List V2, the one confirmed 50-point cost in .planning/CJ_SCHEMA_EVIDENCE.md). At least one of `category` or `keyword` is required. The caller paginates by incrementing `page`; the adapter never loops internally to fetch more than one page per call. `product_list.json` follows the same v2 entries fixture shape and includes a deliberate inert prompt-injection-style title for the same reason `product.json` does. The exact CJ wire field names for this endpoint (`list`, `total`, `pid`/`productNameEn`/`productSku`/`productImage`/`sellPrice`) are inferred by analogy with the confirmed product/variant fields and are not yet verified by an owner-authorized capture; Normalizer fails closed on any other shape rather than guessing permissively.
